...
creditcard.csv                   |
     |                     +-----+-----+
     v                     |   Kafka   |
 +----------+  publish     |   :9092   |     consume     +----------+
 | Producer +------------->|           +---------------->| Consumer |
 | 10 TPS   |   topic:     | topic:    |   poll batches  |          |
 +----------+  transactions|transactions|   of 10 msgs   +----+-----+
                           +-----------+                      |
                                                              | POST /predict
                                                              v
                +---------+      +----------+            +----+-----+
                |  Redis  |<---->|  FastAPI  |<---------->|PostgreSQL|
                |  :6379  | get/ |  :8000   | insert/    |  :5432   |
                |  cache  | set  |          | query      |          |
                +---------+      +----+-----+            +----------+
                                      |
                              GET /metrics
...
### Request flow for a single transaction

```
1. Producer reads a row from creditcard.csv
2. Serializes it as JSON with a UUID, publishes to Kafka topic "transactions"
3. Consumer polls the message from Kafka
4. Consumer POSTs the transaction to FastAPI /predict
5. API checks Redis for cached result (key = txn:{transaction_id})
   --> Cache HIT:  return cached prediction in ~0.1ms
   --> Cache MISS: continue to step 6
6. API scales Time + Amount features using saved StandardScaler
7. API runs XGBoost model, gets prediction + confidence score
8. API writes result to Redis (TTL = 1 hour) and PostgreSQL
9. API returns {is_fraud, confidence, latency_ms, cache_hit}
10. Consumer logs FRAUD ALERT to stdout if is_fraud = true
```
